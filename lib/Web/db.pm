...
sub deleteCookie {
    ...
    $dbh->do("delete from Cookie where cookie_string='$cookie_from_browser'");
}

sub deleteCookieForLogin {
    my ($dbh, $login) = @_;
    $dbh->do("delete from Cookie where fk_login_name='$login'");
}

sub getCookie {
    my ($dbh, $cookie_from_browser) = @_;

    my $st = $dbh->prepare("select fk_login_name from Cookie where cookie_string=?");
...
sub saveCookie {
    ...
    $dbh->do("insert into Cookie(cookie_string, fk_login_name) values('$hash->{cookie_string}', '$hash->{login_name}')");
